...
def bounded_log(a):
    ...
    :param a: number
    :type a:float
    :return: log of a bounded below if a is zero
    :rtype: float
    '''
    if a == 0:
        return B
...
def bounded_log_sum(log_a, log_b, *args):
    ...
    :param log_a: log of number a
    :type log_a: float
    :param log_b: log of number b
    :type log_b: float

    :return: sum of logs bounded below
    :rtype: float
    '''
    log_sum = log_a + log_b
    for ar in args:
...
def online_viterbi_initialization(starting_state):
    ...
    for i in range(K):
        initial_prob.append(bounded_log(initial[i]))
...
def update(t, observation):
    ...
    for j in range(K):
        ...
        for i in range(K):
            aux = bounded_log_sum(p_col.value[i], bounded_log(A[i][j]), bounded_log(E[j][observation]))
            if (aux > max):
                max = aux
                max_index = i
